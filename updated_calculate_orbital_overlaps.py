...
periods_d = range(1,24)
d_longest = 1113.45
f_longest = 1084.16
trials = 10000
#output = open('planets_f_d_overlap_check.txt','w+')
output = open('planets_f_d_overlap_check_updated.txt','w+')
...
def check_two_planets():
	for pf in periods_f:
		...
		chain = np.load('planet_f/calibrated_data/sampler_chain_' + str(pf) + '.npy')
		chain = chain[:,20000:,]
		...
		ecc_chain_f = np.load('planet_f/calibrated_data/eccentricity_posterior_'+str(pf)+'.npy')
		w_chain_f = np.load('planet_f/calibrated_data/omega_posterior_'+str(pf)+'.npy')
		for pd in periods_d:
			h_chain_d = []
			print(d_longest/pd)
			#load semi major axis mcmc chain for d
			chain = np.load('planet_d/calibrated_data/sampler_chain_'+ str(pd) + '.npy')
			chain = chain[:,20000:,]
			...
			ecc_chain_d = np.load('planet_d/calibrated_data/eccentricity_posterior_' + str(pd) + '.npy')
			w_chain_d = np.load('planet_d/calibrated_data/omega_posterior_' + str(pd) + '.npy')
			counter = 0
			for i in range(trials):
				...
				m2 = 0.2#in jupiter masses
				h2 = a2 * (1 - e2) * (m2 / 3.0/ M)**(1.0/3)
				h_chain_f.append(h2 / a2)

				rh_m = 0.5 * (a1 + a2) * ((m1 + m2) / M)**(1.0/3)
				delta = 3.5 * rh_m
				#check if they overlap
				ans1 = check_overlap(a1 + delta,w1,e1,a2,w2,e2)
				if ans1:
					counter += 1
					continue

				ans2 = check_overlap(a1 - delta,w1,e1,a2,w2,e2)
				if ans2:
					counter +=1
					continue
				...
				ac = semi_major_c[r3]
				rh_m = 0.5 * (a1 + ac) * ((m1) / M)**(1.0/3)
				delta = 3.5 * rh_m
				ans3 = check_overlap(a1 - delta,w1,e1,ac,0,0)
				if ans3:
					counter +=1
					continue

				rh_m = 0.5 * (a2 + ac) * ((m2) / M)**(1.0/3)
				delta = 3.5 * rh_m
				ans4 = check_overlap(a2 - delta,w2,e2,ac,0,0)
				if ans4:
					counter +=1
					continue
