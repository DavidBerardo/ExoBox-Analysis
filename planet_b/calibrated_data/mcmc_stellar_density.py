...
def lnprior(p):
	if abs(p[0] - 2457152.2844) > 1 or not(31.704 <p[1] < 31.708) or p[2] > 200 or p[3] > 180 or p[4] > 1:
		return -np.inf
...
p0 = [2457152.2844,15.572,22,88,0.02] + \
	 [0.4551,0.0441] + [-4]*3

err = [0.002,0.001,1,0.5,0.002] + \
	  [0.02,0.03] + [0.5]*3
...
c = sampler.chain
np.save('stellar_density/sampler_chain',c)
f = sampler.flatchain
p_max = f[np.argmax(sampler.flatlnprobability)]
np.save('stellar_density/max_likelihood_point',p_max)
