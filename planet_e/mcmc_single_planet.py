...
data = pd.read_csv('C5_planet_e_only.csv')
...
def lnprior(p):
	if abs(p[0] - 2457142.01656) > 1  or p[1] > 1000 or p[2] > 1000 or p[3] > 180 or p[4] > 1:
		return -np.inf
	if all(i > 0 for i in p[:-1]) and (p[-3] + p[-2] < 1) and (-6 < p[-1] < -2):
		prior_like = 0
...
p0 = [2457142.01656,130,77,89.8,0.03613] + \
	 [0.4552,0.0441] + [-4]

err = [0.004,50,20,0.1,0.01] + \
	  [0.016,0.027] + [0.5]
