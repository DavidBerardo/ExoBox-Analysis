...
for h in range(1,20):
	...
	p0 = [2457166.2629,per,a,89.71,0.02563611] + [0,0] +\
	 [-4]*3

	err = [0.004,0.005,a / 50.0,0.01,0.005] + [0.1,0.1] + \
	  [0.5]*3
	...
	c = sampler.chain
	np.save('stellar_density_eccentricity/sampler_chain_' + str(h),c)
	f = sampler.flatchain
	p_max = f[np.argmax(sampler.flatlnprobability)]
	np.save('stellar_density_eccentricity/max_likelihood_point_' + str(h),p_max)
