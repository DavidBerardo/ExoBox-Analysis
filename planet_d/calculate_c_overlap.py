...
output = open('planets_d_overlap_with_c.txt','w+')
...
def check_two_planets():
	for pd in periods_d:
		...
		chain = np.load('sampler_chain_' + str(pd) + '.npy')
		chain = chain[:,20000:,]
		flat = np.reshape(chain,(len(chain) * len(chain[0]),len(chain[0][0])))
		semi_major_d = flat[:,2]
		...
		ecc_chain_d = np.load('eccentricity_posterior_'+str(pd)+'.npy')
		w_chain_d = np.load('omega_posterior_'+str(pd)+'.npy')
		counter = 0 
		for i in range(trials):
			#draw from mcmc distributions
			r = randint(0,len(semi_major_d)-1)
			a = semi_major_d[r]
			r = randint(0,len(w_chain_d)-1)
			w = w_chain_d[r]
			e = ecc_chain_d[r]
