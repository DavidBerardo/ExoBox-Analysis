...
#C18
data = pd.read_csv('../C18_planet_f_only.csv')
t2 = np.asarray(data['BJD'])
f2 = np.asarray(data['flux'])

#C18 short cadence
data = pd.read_csv('../C18_short_cadence_planet_f_only.csv')
...
def generate_transit(t0,per,a,inc,rp,ecc,w,u):
...
def lnbounds(p):
	if abs(p[0] - 2457186.9145) > 1  or p[2] > 1000 or p[3] > 180 or p[4] > 1 or not(0 < p[5] < 1):
		return -np.inf
...
p0 = [2457186.9145,1084.16 / 1.0,100,89,0.067,0.3,90] + \
	 [0.3]*2 + [-4]*3

err = [0.004,0.05,20,0.5,0.01,0.1,40] + \
	  [0.1]*2 + [0.5]*3
