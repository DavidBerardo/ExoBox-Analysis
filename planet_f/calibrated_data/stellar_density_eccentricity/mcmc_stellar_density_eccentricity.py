...
####LOAD DATA
#C5
data = pd.read_csv('../C5_planet_f_only.csv')
t1 = np.asarray(data['BJD'])
f1 = np.asarray(data['flux'])

#C18
data = pd.read_csv('../C18_calibrated_planet_f_only.csv')
t2 = np.asarray(data['BJD'])
f2 = np.asarray(data['flux'])

#C18 short cadence
data = pd.read_csv('../C18_short_cadence_calibrated_planet_f_only.csv')
...
def generate_transit(t0,per,a,inc,rp,ecosw,esinw):
	params = batman.TransitParams()
	params.per = per
	params.rp = rp
	params.a = a
	params.inc = inc
	params.ecc = np.sqrt(ecosw**2 + esinw**2)
	params.w = np.arctan(esinw/ecosw)
	params.limb_dark = 'quadratic'
	params.u = [0.4551,0.0441]
	params.t0 = t0
...
def calc_rho_ecc(p):
	...
	f = (1 - e**2) / (1 + esinw)
	t14 = (per / np.pi) * np.arcsin(((1 + rp)**2 - a**2 * f**2 * np.cos(inc)**2)**0.5 / (a * np.sin(inc) * f))
	t23 = (per / np.pi) * np.arcsin(((1 - rp)**2 - a**2 * f**2 * np.cos(inc)**2)**0.5 / (a * np.sin(inc) * f))

	G = 2942 # grav constant in units of solar radius^3 / solar mass / day^2
	rho_circ = (2 * rp**0.5 / (t14**2 - t23**2)**0.5)**3 * (3 * per / G / np.pi**2)

	#factor to correct stellar density by
	g = (1 + esinw) / np.sqrt((1 - e**2))
	return g**(-3) * rho_circ
...
def lnlike(theta):
	...
	#light_c5, light_c18, light_c18_short_cad = generate_transit(theta[0],theta[1],theta[2],theta[3],theta[4],theta[5],theta[6],theta[-5:-3])
	light_c5, light_c18, light_c18_short_cad = generate_transit(theta[0],theta[1],theta[2],theta[3],theta[4],theta[5],theta[6])
...
def lnprior(p):
	if abs(p[0] - 2457152.2844) > 1 or not(540 <p[1] < 544) or p[2] > 1000 or p[3] > 180 or p[4] > 1:
		return -np.inf

	#constraints on ecosw/esinw
	if abs(p[5]) > 1 or abs(p[6]) > 1:
		return -np.inf
	
	#if all(i > 0 for i in p[:-3]) and (p[-5] + p[-4] < 1) and all(-6 < j < -2 for j in p[-3:]):
	if all(i > 0 for i in p[:-3]) and all(-6 < j < -2 for j in p[-3:]):
		prior_like = 0
...
#t0, per, a/rs,inc,rp/rs, ecosw, esinw
p0 = [2457186.9145,542.079,232,89.97,0.067] + [0,0] + \
	 [-4]*3

err = [0.004,0.005,0.5,0.01,0.005] + [0.01,0.01] + \
	  [0.5]*3
