...
def generate_transit(t0,per,a,inc,rp,u):
	...
	#m = batman.TransitModel(params,t_combined,supersample_factor = 10, exp_time = 30.0 / 60.0 / 24.0)
	m1 = batman.TransitModel(params,t1,supersample_factor = 5, exp_time = 30.0 / 60.0 / 24.0)
	#m1 = batman.TransitModel(params,t1)
	m2 = batman.TransitModel(params,t2,supersample_factor = 5, exp_time = 30.0 / 60.0 / 24.0)
...
def lnbounds(p):
	if abs(p[0] - 2457186.9145) > 1  or p[2] > 1000 or p[3] > 180 or p[4] > 1:
		return -np.inf
...
#p0 = [2457166.2629,1113.45,119.9,89.7,0.0259] + \
#	 [0.3]*2 + [-4]*3

#err = [0.1,0.1,50,0.5,0.01] + \
#	  [0.1]*2 + [0.5]*3
...
for h in range(11,21):
	p0 = [2457186.9145,1084.16/h,100,89,0.067] + \
		[0.3]*2 + [-4]*3
	...
	c = sampler.chain
	np.save('sampler_chain_' + str(h),c)
	f = sampler.flatchain
	p_max = f[np.argmax(sampler.flatlnprobability)]
	np.save('max_likelihood_point_' + str(h),p_max)
