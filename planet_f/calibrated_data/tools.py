...
2018-07-06 14:05 IJMC: Made compatible with Python3
"""
...
def hparams(params, nbins=10, figsize=[15,10], density=False, normed=False,newfig=True, labs=None, cumulative=False, minorticks=True, plotconf=.683, plotmid=True, color=None):
    ...
    :Options:
      ...
      density sets the histogram normalization (preferred)

      normed sets the histogram normalization (deprecated)
    ...
    # 2018-07-12 08:35 IJMC: Tried to update normed->density
    from numpy import sqrt
    from pylab import figure, gcf, subplot, draw, hist
    from analysis import dumbconf
    ...
    if density is None: density = normed
    ...
    if 'density' in getargspec(hist)[0]:
        histoptions = dict(histtype='step', cumulative=cumulative, density=density)
    else:
...
def loadpickle(filename,mode='both'):
    ...
    if not good:
        try:
            ...
            print('file was never open, but trying another tack')
        try:
            f = open(filename, 'rb')
            ret = pickle.load(f, encoding='bytes')
            good = True
        except:
            print('couldnt even read in binary mode!')
    try:
        f.close()
    except:
...
def sample_3dcdf(cdf, x, y, z, nsamp=1, verbose=False):
    ...
    ret1 = np.zeros(int(nsamp), dtype=y.dtype)
    ret2 = np.zeros(int(nsamp), dtype=x.dtype)
    ret3 = np.zeros(int(nsamp), dtype=x.dtype)
    ...
    while nsampled < nsamp:
        ...
        if prob[0:-1,0:-1,0:-1][ind_x, ind_y, ind_z] > accept_prob[ii]:
...
def roundvals_latex(input, parenmode=False):
    ...
    initial = np.array([roundvals(inp) for inp in input]) 
    output = []
    for tup in initial:
        print( tup)
        if len(tup)==2 or (tup[1]==tup[2]):
            if parenmode:
                ndigit = 2
                if max([float(tupp) for tupp in tup[1:]])>10:
                    ndigit = int(np.ceil(np.log10( max([float(tupp) for tupp in tup[1:]])  )))
                unc = tup[1][-ndigit:]
